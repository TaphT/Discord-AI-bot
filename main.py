...
async def on_message(message):
    if 'insert.prefix' in message.content:
        global working
        craiyon_embed = discord.Embed(title=f"Generating{message.content.replace('insert.prefix','')}", url="https://craiyon.com",
                                      description="Craiyon is thinking.. (This may take up to 3 minutes)",
                                      color=0xffae00)
        wait = await message.reply(embed=craiyon_embed)
        print(f"Requesting {message.content.replace('insert.prefix','')}")
        try:
            if working == False:
                working = True
                get_image(message.content.replace('insert.prefix',''))
    ...
    if message.content == 'c!help':
        help_embed = discord.Embed(title="Craiyon Bot", url="https://craiyon.com",
                              description="Generates Images From https://craiyon.com | Prefix is c!help   `insert.prefix` - Generates an image based on a word  e.g `insert.prefix tree` will generate a tree on https://craiyon.com",
                              color=0xffae00)
